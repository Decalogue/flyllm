...
**一句话总结**：**排序**后固定最左元素 `nums[i]`，在右侧用**双指针**找两数之和为 `-nums[i]`；相等则记录，并**跳过重复**的 `i / left / right`。

**关键词**：`排序` `双指针` `去重三处` `剪枝`

### 复杂度速记

- **时间**：O(n²)（排序 O(n log n) 被吸收）；**空间**：O(1) 额外（不计输出与排序栈）。

### 记忆口诀

「先排后定一，左右夹逼和；命中先存档，再挤掉重复。」
...
1. **不去重**  
   - 会输出大量重复三元组；需在 **`i` 层**与**命中后 `left`/`right` 移动**两处跳过相同值。

2. **去重过早**  
   - 在**未找到合法三元组前**错误地跳过与 `left`/`right` 相等的元素，可能漏解（如全零 `[0,0,0]`）。

3. **把两数之和模板硬套**  
   - 两数之和常用哈希；三数之和要**不重复三元组**，排序 + 双指针通常比哈希更省事。

4. **目标和不是 0 的变体**  
   - 本题 target 为 0；若为 `target`，则双指针找的是 `target - nums[i]`，其余相同。
